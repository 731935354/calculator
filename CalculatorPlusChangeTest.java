...
public class CalculatorPlusChangeTest extends JFrame implements ActionListener{
	
	/**
	 * Matrix使用字典{'Clear':清空,'Add':加法,'Dif':减法,'Mul':乘法,'Rev':可逆矩阵,'Det':行列式值,
	 * 'Eig':矩阵特征值,'Tra':转置矩阵,'UpT':上三角，Rank':秩,'Anster'：加减乘法的等号.}
	 */
	private static final long serialVersionUID = -2744739512640090752L;
	/**
	 * Science界面
	 */
	//AC与退格键
    ...
    JTextArea textArea = new JTextArea ( "input matrix here:"+"(example)\n"
    		 +"1 2 3;\n"+"4 5 6;\n"+"7 8 9\n"+"Before you use it,please clear it");
    //JTextArea textArea = new JTextArea("input matrix here 1234:", 10, 10);
    // 按钮
	...
	public CalculatorPlusChangeTest() {
        super();
        // 初始化计算器
        init();
        // 设置计算器的背景颜色
    ...
    private void handleDet(){
    	 ...
    	 * for(int i = 0;i <det.length;i++){
    		for(int j = 0; j< det[0].length;j++){
    			System.out.println(getMatrixfromText()[i][j]);
    		}*/
    	
    	textArea.setText(String.valueOf(Det(getMatrixfromText(),getMatrixfromText().length-1)));
    	//System.out.println(Det(getMatrixfromText(),getMatrixfromText().length-1));
    }
    //处理Rank健被按下的按钮
   ...
   */
    ...
    public static void main(String[] args)
	{
	    ...
	    new CalculatorPlusChangeTest().setVisible(true);
	}
